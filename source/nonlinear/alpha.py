...
def alphaComp(taskSet,coreNum,alg):
    ...
    for task in taskSet:
        x = x+task[0]
        assignCoreNum = max([math.floor(alpha*coreNum),1])                ##########学学学###########
        i = assignCoreNum                                                 ########学学学#############
        for i in range(assignCoreNum-1,coreNum-1):                        #########学学学############
            if cores[i] < cores[i+1]:
                #assignCoreNum = i+1 #min([i+1,math.ceil(task[2])])
                ##print("assgin core is ",assignCoreNum)
                break
        assignCoreNum = min([i+1,math.floor(task[2]),coreNum])              ##########学学学###########
        ##print("task exe",task[3]/assignCoreNum, "Assigned cores", assignCoreNum,"x", x)
        for i in range(assignCoreNum):                                    #########学学学############
            nonlinear = twophare(assignCoreNum, task[2])  # ????????wowowowowowowowoowowowowowowowo
            cores[i] = max([x,cores[assignCoreNum-1]]) + task[3]/(assignCoreNum*nonlinear)    #############2学学学#############
        cores = sorted(cores)
        ##print("core", cores)
    ##print("offloading = ",x)
