...
def plot():
    ...
    # plt.xlabel(r"$(a)\ linear-speadup\ processors$", size=n)
    # plt.xlabel(r"$(a)\ linear-speadup\ jobs$", size=n)
    # plt.text(25,0.7, r'$(b)\ linear,n=50,β=0.5$')fontdict
    plt.text(14, 0.45, r"$(a)\ linear-speadup\ jobs$",size=n)

    # plt.text(30, 0.7, r'$(a)\ linear$')
    ...
    # 生成折线图
    plt.plot(x, gV.arr_avg[0],"+-.",color='g',linewidth=2,label='Impt-I')
    plt.plot(x, gV.arr_avg[1], "+--", color='red',linewidth=2,label='Impt-I*')
    plt.plot(x, gV.arr_avg[2], "+-", color='blue',linewidth=2,label='Impt-II')
    plt.plot(x, gV.arr_avg[3], "+:", color='y',linewidth=2,label='Impt-B')
    # font1 = {'family': 'Times New Roman', 'weight': 'normal', 'size': 13.3, }
    plt.legend(loc='best', ncol=2,fontsize=n)
    # plt.legend(loc='best', ncol=4, prop=font1)
    ...
    plt.savefig("./figure/Fig.5(a).png")
    plt.show()